...
let data: Array<any>
try {
  const mockData = readFileSync(resolve('app/athletes.json'), {
    encoding: 'utf-8',
  })
...
const searchMockData = (searchString: string) => {
  ...
  return data.filter(
    (record) =>
      ...
      (record.firstName &&
        record.firstName.toLowerCase().includes(lowerCaseSearch)) ||
      (record.lastName &&
        record.lastName.toLowerCase().includes(lowerCaseSearch)) ||
      (record.team &&
        record.team.name &&
        record.team.name.toLowerCase().includes(lowerCaseSearch)) ||
